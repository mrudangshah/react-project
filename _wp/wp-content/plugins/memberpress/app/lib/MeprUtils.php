...
class MeprUtils {
  ...
  public static function months( $n, $month_ts = false, $backwards = false, $ts_day = false ) {
    ...
    $month_num      = gmdate( 'n', $month_ts );
    $day_num        = ( ( int ) $ts_day < 1 || ( int ) $ts_day > 31 ) ? gmdate( 'j', $month_ts ) : $ts_day;
    $year_num       = gmdate( 'Y', $month_ts );
    $hour_num       = gmdate( 'H', $month_ts );
    $minute_num     = gmdate( 'i', $month_ts );
    $second_num     = gmdate( 's', $month_ts );
    ...
    $calc_date = new DateTime( "{$year_num}-{$month_num}-1 {$hour_num}:{$minute_num}:{$second_num}" );
    ...
    return abs( $calc_date->getTimestamp() - $month_ts );
  }
  ...
  public static function calculate_proration_by_subs($old_sub, $new_sub, $reset_period=false) {
    ...
    if(($coupon = $new_sub->coupon()) && $coupon->discount_mode=='first-payment') {
