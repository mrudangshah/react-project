...
class BP_REST_Members_Endpoint extends WP_REST_Users_Controller {
	...
	public function user_data( $user ) {
		...
		$xpath_full = new DOMXPath(@DOMDocument::loadHTML($author_avatar_full));
        $xpath_thumb = new DOMXPath(@DOMDocument::loadHTML($author_avatar_thumb));
		...
		$data['user_role'] = $user_role->roles;
