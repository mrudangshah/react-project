...
function tpw_get_members_cat_custom_tax() {
...
function tpw_get_posts_by_member_cat() {

  if( tpw_get_members_cat_custom_tax() ){

    // store the custom tax collections we have
    $custom_tax_collection = tpw_get_members_cat_custom_tax();
    ...
    foreach ($custom_tax_collection as $key => $tax) {
      ...
      foreach ($tax_terms as $key => $tax_term) {
         ...
         *
         */
        register_rest_route( 'tpw-rest-endpoints/v1', '/post/members/(?P<id>\d+)/cat/(?P<slug>\S+)', array(
          'methods' => 'GET',
 ...
 *
 */
add_action( 'rest_api_init', 'tpw_get_posts_by_member_cat' );