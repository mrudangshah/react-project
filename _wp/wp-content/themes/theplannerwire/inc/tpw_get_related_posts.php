...
function tpw_get_related_custom_tax() {
...
function tpw_get_related_posts() {

  if( tpw_get_related_custom_tax() ){

    // store the custom tax collections we have
    $custom_tax_collection = tpw_get_related_custom_tax();
    ...
    foreach ($custom_tax_collection as $key => $tax) {
      ...
      foreach ($tax_terms as $key => $tax_term) {
         ...
         *
         */
        register_rest_route( 'tpw-rest-endpoints/v1', '/post/related/(?P<id>\d+)', array(
          'methods' => 'GET',
 ...
 *
 */
add_action( 'rest_api_init', 'tpw_get_related_posts' );