...
class Single extends Component {
  ...
  render() {
    ...
    if( typeof parsed === 'object'){
      if(ispathName !== 'the-spread' && ispathName !== 'sponsored'){
      ...
      if(ispathName === 'sponsored'){
        // var image_or_video = parsed.image_or_video.value
        // var single_image_or_gallery = parsed.single_image_or_gallery.value 
      }
    }
        ...
        <section id="post_dtls" className="onthewire">
          ...
          <div className="inner_postdtls">
            <div className="image_block">
              <div className="imgbox">
                { 
                  singleData.acf.image_or_video.value  === 'video' ?
                  <video controls>
                    <source src={singleData.acf ? singleData.acf.sponsored_video ? singleData.acf.sponsored_video.url:'' : ''} type="video/mp4"></source>
                  </video>
                  : singleData.acf.image_or_video.value  === 'image' ?
                    
                    singleData.acf.single_image_or_gallery.value  === 'featured_image' ?
                      !isLoading && <span>
                        <img src={ siteurl + IMAGE.trans_915 } alt="" />
                        <img className="absoImg" src={ singleData.acf ? singleData.acf.single_featured_image? singleData.acf.single_featured_image.url:sample_image:'' } alt={ 'Sponsored' } />
                        </span>

                    : singleData.acf.single_image_or_gallery.value  === 'image_gallery' ?
                      singleData.acf.image_gallery.length > 0 ? 
                      <ul>
                      <Slider {...settings}>
                        { singleData.acf.image_gallery.map((item) => {
                        return <li>
                          <div className="imgbox">
                            <img src={ siteurl + IMAGE.trans_915 } alt="" />
...
const mapStateToProps = state => ({
  ...
  single: state.singleData.singleItems,
  postID: state.singleData.pID,
  comments: state.singleData.comments,
})
